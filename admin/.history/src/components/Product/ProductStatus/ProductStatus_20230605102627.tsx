...
function ProductStatus() {
  ...
  useEffect(() => {
    axios.get('http://vikrant.store/test/api/product/getList/byDisplay', {
      params: {
        limit: 8,
        page: 1,
  ...
  const handleDeleteProducts =  (itemCode: string) => {
    ...
    axios.get('http://vikrant.store/test/api/product/delete', {
       params: {
         itemCode
       }
    })
  ...
  const handleInActivateProduct = (item: Item) => {
    ...
    axios.get('http://vikrant.store/test/api/change/displayStatus', {
      params: {
        status: 'HIDDEN',
