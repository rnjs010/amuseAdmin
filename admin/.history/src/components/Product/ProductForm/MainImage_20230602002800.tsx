...
function MainImage({option, mainImgProp, onAdd, onRemove}:ExtraInfoProps) {
  ...
  return (
    <div className={`${styles.container} ${styles.mainImgContainer}`}>
      <span className={` ${styles.title} ${styles.mainImg}`}>메인 이미지 (4개 이상 첨부)</span>
      <input className={styles.mainImageInput} id="mainImgInput" onChange={handleMainImg} accept="image/png, image/jpeg" multiple type="file"/>
      <div>
        {renderImageList()}
      </div>
    </div>
