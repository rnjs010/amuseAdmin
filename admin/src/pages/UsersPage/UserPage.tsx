...
import { CBadge, CButton, CCallout, CCard, CCardBody, CCol, CContainer, CForm, CFormInput, CModal, CModalBody, CModalHeader, CPagination, CRow } from "@coreui/react";
...
const UserPage=()=>{
    ...
    const [slicedArray, setSlicedArray] = useState(
      slicingArray(users, slicingNumber)
    ); // page number 클릭시 보여주는 list
    ...
    const userRankModalControll = (user:any)=>{
        setModalStatus(true)
        setEdit(<EditView user={user} count={count} setCount={setCount} modalStatus={modalStatus} setModalStatus={setModalStatus}></EditView>)
    }
      ...
      <CContainer fluid style={{margin:40,position:"relative"}}>
        <CCol style={{marginBottom:40}}>
          <CCallout color="info" style={{marginBottom:12, marginLeft:4}}>
            <small className="text-muted" style={{marginRight: 12}}>고객 계정 수</small>
              ...
              <Table striped bordered hover>
                ...
                <tbody>
                  {/* {slicedArray[pageNumber - 1].length > 0 &&
                    ...
                    {slicedArray.length > 0 && slicedArray[pageNumber - 1].length > 0 &&
                    slicedArray[pageNumber - 1].map((item, index) => (
                      <tr key={index} onClick={()=>{userRankModalControll(item)}}>
                        <td>{index + 1}</td>
                        <td>{item.name}</td>
                        <td>{item.email}</td>
                        <td>{item.grade}</td>
        ...
        <div style={{ marginTop: 10 }}>
            <Paging
                ...
                minWidth={"650px"}
                num={pageNumber}
                setNum={setPageNumber}
            />
        </div>
        {
            modalStatus && 
                <div style={{position:"absolute",top:0,width:"100%",height:"100%",backgroundColor:"#fff",zIndex:10,borderRadius:12}}>
                    <ButtonWrapper onClick={userRankModalCancel}>취소</ButtonWrapper>
                    <div style={{position:"sticky",top:15,marginLeft:"50px",marginTop:"65px" ,zIndex:99}}>
                        {edit}
                    </div>
                </div>
        }
