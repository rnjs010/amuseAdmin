...
function TicketModal({onSave, onToggle}: MordalProps) {
  ...
  const [weekdayPrices, setWeekdayPrices] = useState(price.weekdayPrices);
  const weekDays: Weekday[] = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];

  useEffect(() => {
    const newWeekdayPrices = {...price.weekdayPrices};
    ...
    for (let weekday of weekDays) {
      ...
      if (!isWeekdayExists) {
        newWeekdayPrices[weekday] = '0';
      }
    }
  
    setPrice((prev) => ({
      ...prev,
      weekdayPrices: newWeekdayPrices
    }));
  
  }, [price.startDate, price.endDate]);


const renderValidWeekDays = () => {
    let newWeekdayPrices = {...weekdayPrices};
      ...
      return (
        <div className={styles.weekdayPriceInput} key={weekday}>
          ...
          <input
            ...
            className={`${isWeekdayExists? styles.weekDayPrice : styles.disabled_weekDayPrice}`}
